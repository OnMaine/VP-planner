<template>
  <section v-if="planStore.massConfig.useSpamNobles" class="panel">
    ...
    <div class="action-row">
      <button class="btn btn-primary" @click="addEmpty">+ Вручну</button>
      ...
      <span v-if="planStore.spamNobleTargets.length" class="target-count" style="margin-left:auto">
        {{ planStore.spamNobleTargets.length }} цілей
      </span>
    </div>

    <div v-if="bulkOpen" class="bulk-panel">
      <p class="bulk-hint">Координати цілей — по одній або через пробіл/кому/новий рядок</p>
      <div class="bulk-row">
        <textarea v-model="bulkText" class="bulk-textarea" rows="4" placeholder="500|500&#10;501|501" />
        <div class="bulk-time">
          <label>Дата і час прильоту
    ...
    <p v-else class="muted-text" style="margin-top:0.5rem">Цілей ще немає.</p>
  </section>
</template>
...
function doBulkAdd(): void {
  ...
  if (isNaN(arrivalTime.getTime())) { bulkError.value = 'Вкажіть час прильоту'; return }
  const matches = bulkText.value.match(/\d{1,3}\|\d{1,3}/g)
  if (!matches?.length) { bulkError.value = 'Не знайдено жодних координат'; return }
  let added = 0
